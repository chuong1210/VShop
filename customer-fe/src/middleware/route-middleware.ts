...
const routeMiddleWare = (req: NextRequest) => {
	...
	const expiresAtCookie = req.cookies.get(cookieConfig.expires_at)?.value;

	const isLogIn =
	  expiresAtCookie &&
	  differenceInDays(parseInt(expiresAtCookie), new Date()) > 0;
