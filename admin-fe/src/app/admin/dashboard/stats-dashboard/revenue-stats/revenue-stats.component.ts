...
import { IStatsRevenue, IStatsSelling } from 'data/responses/stats/stats.reponses';
...
export class RevenueStatsComponent {
	...
	async updateChart() {
		...
		try {
			const success = await lastValueFrom(this.statsService.getRevenue(param));
	...
	updateChartForYear(data: IStatsRevenue[]) {
		const monthlyData = new Array(12).fill(0);
		data.forEach((item) => {
			if (item.time >= 1 && item.time <= 12) {
				monthlyData[item.time - 1] = item.revenue;
			}
		});
	...
	updateChartForMonth(data: IStatsRevenue[]) {
		const daysInMonth = Math.max(...data.map((item) => item.time || 0));
